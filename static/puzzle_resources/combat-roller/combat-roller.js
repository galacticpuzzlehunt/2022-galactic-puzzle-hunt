...
const FIGHTER_1_CUSTOM = document.getElementById('fighter1-custom');
const FIGHTER_2_CUSTOM = document.getElementById('fighter2-custom');
const FIGHTER_1_RADIOS = document.getElementById('practice-form').elements['fighter1'];
const FIGHTER_2_RADIOS = document.getElementById('practice-form').elements['fighter2'];
const CAMPAIGN_OUTPUT_DIV = document.getElementById('campaign-output');
const CAMPAIGN_WORD = document.getElementById('campaign-word');
...
function getSelectedRadioValue(inputName) {
...
function setVisibility(el, shouldShow) {
    el.style.display = shouldShow ? BLOCK : NONE;
}
function resetErrorDisplay(el) {
    el.innerText = DEFAULT_ERROR_MESSAGE;
    setVisibility(el, false);
}
function calculateGoodStyle(goodValue) {
    return goodValue === true ? GOOD : (goodValue === false ? BAD : NEUTRAL);
}
function makePreFightAnalysis(data) {
...
function roundedPercent(num) {
    return `${Math.round(num * 1000 / 4.410) / 1000}`;
}
function probabilities(your_word, oppo_word) {
    let win = 0;
    let draw = 0;
    let loss = 0;
    for (let i = 0; i < 6; i++) {
...
function getPracticeData(your_word, oppo_word) {
    let errors = '';
    ...
    const [win, draw, loss] = probabilities(isPreset(your_word) ? PRESET_FIGHTERS[Number.parseInt(your_word) - 1] : your_word, isPreset(oppo_word) ? PRESET_FIGHTERS[Number.parseInt(oppo_word) - 1] : oppo_word);
...
function fightCampaignStage(your_word, oppo_word) {
...
function wordValidationMessage(s) {
    if (s.length !== 6) {
...
function isPreset(s) {
    ...
    return ((_b = (_a = s.trim().match(new RegExp(regexString))) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0) > 0;
}
function presetOrWordValidationMessage(s) {
    if (hasNumber(s)) {
        if (!isPreset(s)) {
...
function practice(e) {
    ...
    const data = getPracticeData(fighter1 === CUSTOM ? normalize(FIGHTER_1_CUSTOM.value) : fighter1, fighter2 === CUSTOM ? normalize(FIGHTER_2_CUSTOM.value) : fighter2);
    try {
        const prob1 = data.prob1;
        const prob2 = data.prob2;
        const probDraw = data.probDraw;
...
function onCampaignBroadcast(message) {
    let data = message;
