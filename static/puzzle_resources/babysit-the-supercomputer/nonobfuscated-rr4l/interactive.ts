...
const dcd9 = document.getElementById('gr2d') as HTMLInputElement;
...
const p6mj = document.getElementById('fayx') as HTMLButtonElement;
const h8gd = document.getElementById('c87wl') as HTMLInputElement;
const a6bi = document.getElementById('ankf') as HTMLSpanElement;
const rdep = document.getElementById('dzla') as HTMLButtonElement;
...
// Needed because z76a gets loaded in from a separate file.
declare const r9a2: readonly string[];
const z76a = new Set<string>();
 ...
 */
function w7xpz(value: number): (input: number) => number {
...
let cu83: boolean = false;
let frozenStartTimestamp: number = Date.now();

// Start timestamp of the program
let l81pq: number|undefined;
let r2ci: number|undefined;
...
const o4tyr = new Map<string, [number, number]>();

// Create all the 'timers' for ongoing tasks.
let g2u7: [string, number, number, HTMLParagraphElement][] = [];

// The current content of the computer.
let sh1z: string = '';

// Program of the computer.
const wgs0 = new Map<number, string[]>();
...
function t8opn(g: number): string {
  return ('0' + Math.floor(g)).slice(-2);
}

function kihe(seconds: number): string {
...
function g9qgm(s: string, a: number): string {
  return e905.charAt((e905.indexOf(s) + a + e905.length) % e905.length)
}

function jybv(k: string): string {
  let res = '';
...
function d5490(c: string): string {
  let a = '';
...
function ttla(h: string): string {
  return h.replaceAll('B', 'AM');
}

function zwqq(n: string): string {
  const j = n.length - 1;
  if (n.length > 3) {
    const i = n.length - 4;
...
const x8og: Map<string, (w: string) => string> = new Map([
  [Task.DELTA, jybv],
  [Task.LAMBDA, d5490],
  [Task.ETA, ttla],
  [Task.TFC, zwqq]
...
function h2zhy(): boolean {
  const j = dcd9.value;
  return j.length > 0 && z76a.has(j.toLowerCase());
}

function ns3t(): boolean {
  return h2zhy() && h8gd.value.length > 0;
}

function r2cs(): HTMLTableCellElement {
  return document.createElement('td');
}

function r57s(): boolean {
  try {
...
const u1j9 = new Map<string, number>();
fcm6.forEach(([u7fv, _], pafw) => {
  u1j9.set(pafw, u7fv);
})

function kf5s(k: string): string {
  if (goh3(k)) {
    return k0i7;
  } else if (!p8ga(k)) {
    return jo73;
...
function c0n2(y: HTMLButtonElement, r: boolean): void {
  if (r) {
...
function cidq(): void {
  ...
  if (g) {
    g.forEach((q) => {
      const l = h2xv4(q as HTMLSpanElement);
      const f = l.innerText;
...
function wfea(p: string, l: number): void {
  u1j9.set(p, l);
  const timeTd = ii73(p);
...
function sou0(b: HTMLButtonElement): void {
  b.innerText = sxeh;
  setTimeout(() => b.innerText = rit7, 1000);
}

// Which tasks affect other tasks.
const m80lz: Map<string, Map<string, (input: number) => number>> = new Map([
...
function qdpy(e: HTMLButtonElement): void {
...
function o58xh(s: number): string[] {
  const x1ybrs: [string, number, number, HTMLParagraphElement][] = [];
...
function t12o(x: string, u: number, s: number): void {
  const b = u1j9.get(x);
  const c = qno1(x, b, u);
  g2u7.push([x, b, u, c]);
  o4tyr.set(x, [s, b]);
...
function qno1(w: string, o: number, k: number): HTMLParagraphElement {
...
function sc7n(u: string, m: number): HTMLSpanElement {
...
function e0v2w(s: string): string {
  return `${s}-task-status`;
}

function scyp(s: string): string {
  return `${s}-task-time`;
}

function mtl7(p: string, v: number): void {
  const c = u1j9.get(p);
...
function y4pgt(p: HTMLParagraphElement): HTMLSpanElement {
  return p.getElementsByClassName(e729)[0] as HTMLSpanElement;
}

function h2xv4(s: HTMLSpanElement): HTMLButtonElement {
  return s.getElementsByTagName('button')[0];
}

function fsnt(n: string): HTMLTableCellElement {
  return document.getElementById(e0v2w(n)) as HTMLTableCellElement;
}

function ii73(d: string): HTMLTableCellElement {
  return document.getElementById(scyp(d)) as HTMLTableCellElement;
}

function d98m1(k: boolean) {
...
async function t7bm(): Promise<void> {
  try {
...
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') jqwb() });
...
registerOnGlitchHook((ex) => {
  cu83 = true;
  Array.from(document.getElementsByClassName(b4730) as HTMLCollectionOf<HTMLElement>).forEach((el) => {
